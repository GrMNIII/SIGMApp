...
const initialNewCrackState: Omit<CrackData, "id" | "project_id"> = {
  ...
  gps_lon: null,
  elemento_tipo: null,
  elemento_material: null,
  elemento_espesor_cm: null,
  grieta_orientacion: null,
  grieta_longitud_visible_m: null,
  grieta_ancho_inicial_mm: null,
  grieta_clasificacion_preliminar: null,
  instrumentacion_modelo: null,
  instrumentacion_n_serie: null,
  instrumentacion_resolucion_mm: null,
  instrumentacion_eje_x: null,
  ...
  instalacion_foto: null,
  instalacion_observaciones: null,
  umbral_verde_mm_sem: null,
  umbral_amarillo_mm_scm: null,
  umbral_rojo_mm_scm: null,
};
        ...
        >
          <Text
            style={[
              ...
              currentValue === dir &&
                directionSelectorStyles.buttonTextSelected,
            ]}
          >
            {dir.charAt(0)}
          </Text>
    ...
    <View>
      <View style={formStyles.inputGroup}>
        ...
        <View style={installationDateTimeStyles.dateTimeContainer}>
          ...
          />
          <TouchableOpacity
            style={installationDateTimeStyles.dateTimeButton}
...
export default function CrackCreate() {
  ...
  const [newCrackData, setNewCrackData] = useState<Omit<
    CrackData,
    "id" | "project_id"
  > | null>(null);
  ...
  const handleInputChange = (
    key: keyof typeof initialNewCrackState,
    text: string
  ) => {
  ...
  const handleOrientationSelect = (orientation: string) => {
    setNewCrackData((prev) => {
      ...
      const newValue =
        prev.grieta_orientacion === orientation ? null : orientation;
            ...
            [
              {
                ...
                onPress: () =>
                  router.replace({
                    ...
                    params: { projectId, crackId: cId },
                  }),
              },
            ]
          );
        } else {
          setNewCrackData(initialNewCrackState);
          setExistsCheckDone(true);
        }
      } catch (err) {
        console.error("Error al verificar existencia:", err);
        setError("Error al verificar la existencia local de la grieta.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleSaveCrack = async () => {
    if (!newCrackData || !newCrackData.name) {
      Alert.alert("Error", "El campo 'Nombre de la Grieta' es obligatorio.");
      return;
    }
    ...
    try {
      ...
      if (exists) {
        crackService.update(crackId, crackPayload);
        Alert.alert(
          "Actualizado",
          `Grieta "${crackPayload.name}" actualizada correctamente.`
        );
      } else {
        crackService.create(crackPayload);
        Alert.alert(
          "Éxito",
          `Grieta "${crackPayload.name}" creada localmente.`
        );
      }
      ...
      console.error("Error al guardar grieta:", err);
      Alert.alert(
        "Error",
        "No se pudo guardar la grieta en la base de datos local."
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.header}>
          Verificando existencia de la grieta {crackId}...
        </Text>
    ...
    >
      <ScrollView style={styles.container}>
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("grieta_clasificacion_preliminar", text)
          }
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("grieta_longitud_visible_m", text)
          }
          keyboardType="numeric"
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("grieta_ancho_inicial_mm", text)
          }
          keyboardType="numeric"
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("instrumentacion_n_serie", text)
          }
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("instrumentacion_resolucion_mm", text)
          }
          keyboardType="numeric"
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("instrumentacion_lectura_cero", text)
          }
          keyboardType="numeric"
        ...
        <FormInput
          ...
          onChange={(text) =>
            handleInputChange("instalacion_observaciones", text)
          }
