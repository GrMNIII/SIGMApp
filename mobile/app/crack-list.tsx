...
export default function CrackList() {
    ...
    // Función para obtener la lista de grietas del proyecto
    const fetchCracks = useCallback(async (pId: string) => {
        setLoading(true);
        setError('');
        
        try {
            // Llama al endpoint de listado: GET /cracks?projectId=:projectId
            const res = await api.get(`/cracks?projectId=${pId}`);
            setCracks(res.data);
        } catch (err) {
            console.error("Error al cargar lista de grietas:", err);
            setError('No se pudieron cargar las grietas de este proyecto. Revisa el servidor.');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.listContainer}>
                {cracks.length === 0 ? (
                    <Text style={styles.emptyText}>No hay grietas registradas en este proyecto.</Text>
                ) : (
                    cracks.map((crack) => (
                        <TouchableOpacity
                            key={crack.id}
                        ...
                        >
                            <Text style={styles.crackTitle}>{crack.name} ({crack.id})</Text>
                            <Text style={styles.crackSubtitle}>Área: {crack.edificio_area || 'N/A'}</Text>
                        </TouchableOpacity>
                    ))
                )}
            </ScrollView>
